	...
	\section{Présentation de l'organigramme}
		...
		\begin{center}\begin{tikzpicture}
			\begin{scope}[xscale=2,yscale=1.5]	
				...
				\node (API) at (-2,3) [rectangle,draw,fill=blue!25,text depth=7cm,minimum width=5cm,minimum height=6cm,font=\textbf\Large] {\begin{tabular}{c}API\end{tabular}};
				\node (CHRG1) [rectangle,draw,dashed,fill=white] at ([yshift=1.5cm]API.center){\begin{tabular}{c}Chargement\\des données\end{tabular}};
				\node (CHRG2) [rectangle,draw,dashed,fill=white,below=of CHRG1.south,yshift=0.8cm] {\begin{tabular}{c}Chargement\\des données\end{tabular}};
				\node (ADD1) [rectangle,draw,dashed,fill=white,below=of CHRG2.south,yshift=0.8cm] {\begin{tabular}{c}Chargement\\des données\end{tabular}};
				\node (ADD2) [rectangle,draw,dashed,fill=white,below=of ADD1.south,yshift=0.8cm] {\begin{tabular}{c}Analyse descriptive\\des données\end{tabular}};	
				...
				\draw (-2,6) -- (API);
				\draw (2,6) -- (IG);
				\path[->,>=stealth'] (IG) edge[bend left=-20] node[anchor=south,above]{(1)} (CHRG1);
				\path[->,>=stealth'] (CHRG1) edge[bend left=0] node[anchor=south,above]{(2)} (IG);
				\path[->,>=stealth'] (IG) edge[bend left=0] node[anchor=south,above]{(3)} (ADD1);
				\path[->,>=stealth'] (ADD1) edge[bend left=-20] node[anchor=south,above]{(4)} (IG);
			\end{scope}
			%Légende
			\begin{scope}
