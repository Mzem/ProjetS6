...
\newpage\clearpage\pagenumbering{arabic}
	...
	% Quelqu'un fait l'annonce de plan svp
	% 1 architecture de l'application : organigramme, donnnées qui criculent entre les modules, fonctionnalités
	% 2 contraintes et choix des outils de programmation
	% 3 fonctionnement de l'application
	% 4 points délicats
	% 5 bilan technique
	% 6 bilan de l'organisation interne
	% 7 bilan comparatif des coûts présumés et des coûts finaux de notre produit
	
		\paragraph{}Dans une première partie de ce document, on présentera l'architecture de notre application, illustrée par un organigramme qui a été préalablement établi. Cet organigramme représente un découpage de l'application en modules avec les différentes informations qui circulent entre ces modules.\\
	...
	% nos trois dernieres parties ont aussi un rôle conclusif, donc on peut faire très court pour notre vraie conclusion.
	\section{Architecture de l'application}
		...
		On notera les arrangements faits dans la représentation des informations transmises entre les modules de l'organigramme. Ces arrangements sont principalement effectués pour des fins de compatibilité avec les outils utilisés et seront explicitement justifiés.
		\begin{figure}[H]
			\begin{tikzpicture}
		...
		%Malek stp, corrige ça ou augmente les marges du cahier : Cette partie a aussi besoin d'un peu de mise en page, il y a trop de marge à droite a cause des enum + itemize + description)
	...
	% Si vous voyez des trucs pertinents à dire dedans allez-y, on peut se baser majoriairement sur cette partie pour justifier nos problèmes, donc ne pas tout réexpliquer en dessous !
	
	\section{Fonctionnement de l'application}
	% On avait oblié cette partie au début, elle est dans les recommandations
	...
	% J'ai retiré un bout de la partie compportement, pcq elle est faite plus précisément au-dessus
		...
		%des tests unitaires l'indépendance c'est le critère de base, pas besoin de réexpliquer ça.
		%on détaille après quels pbs exactement ils ont aidé à résoudre.
		...
		\subsection{Problèmes rencontrés}
			...
			%J'ai retiré, points divergents, j'ai expliqué quand tu parles des trucs serialisables de flask. tout vient de la, et le nom d'une extension c'est pas grave haha (j'espère), mais c'est as la peine de le mentionner
