...
\newpage\clearpage\pagenumbering{arabic}
	...
	\section{Package Interface web}
		Ce package est composé de quatre modules. L'un d'eux aura pour fonction de lancer l'application, ainsi que de gérer les informations et les erreurs transitant entre les différents modules des packages. Les trois autres contiendront les fonctionnalités que leur fenêtre devra avoir.
		...
		\begin{enumerate} 
				...
				\vspace{1em}\item Fonctionnalité pour générer la fenêtre résultats ADD.
					...
					\underline{Paramètres :}
						...
						\end{description}
					\underline{Retour :} \lstinline!render_template()! : fonction qui renvoie le contenu de la page web mis en paramètre qui va être envoyée au client. \\
					\underline{Description :} cette fonction correspond à la requête HTTP (c'est à dire le chemin de la page demandée) envoyée par le client et reçue par le serveur qui sera soit un GET soit un POST. Pour cela il faudra indiquer avant de déclarer la fonction la ligne suivante : \lstinline!@app.route('/resultat_ADD/', methods=['GET', 'POST'])!. Elle retournera le contenu de la page web mis en paramètre de la fonction \lstinline!render_template()! associé au code \lstinline!HTML! de la fenêtre résultat ADD.
					
				\vspace{1em}\item \colorbox{yellow}{Surement inutile => en cours de réfléxion} Fonctionnalité pour vérifier que la vérification du format est OK.
					\begin{lstlisting}
						verifFormatOK(ouverture)
					\end{lstlisting}
					\underline{Paramètres :}
						\begin{description}[style=unboxed,leftmargin=0.2cm]
							\item \lstinline!ouverture! : \lstinline!int! - correspond à la valeur de retour de \lstinline!ouvrir(chemin)!
						\end{description}
					\underline{Retour :} variable de type entier signalant un succès (0) ou un échec (1).\\
					\underline{Description :} cette fonction prend en paramètre un entiers correspondant à la sortie de la fonction \lstinline!ouvrir(chemin)! du module "Vérification format fichier". Si la fonction de ce module n'a détecté aucune erreur alors elle renverra 0, sinon 1.
		...
		Ce module est la fenêtre qui va permettre à l'utilisateur d'aller charger le fichier CSV. Il y aura 3 fonctionnalités :
			...
			\begin{enumerate}
				...
				\paragraph{}Les deux fenêtres abordent une apparence simple et épuré. L'effet cherché et que lorsque l'utilisateur regardera la fenêtre, il devra tout de suite comprendre comment fonctionnera la fenêtre.
